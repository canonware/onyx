...
void
nxo_l_name_print(cw_nxo_t *a_thread)
{
	cw_nxo_t		*ostack, *depth, *nnxo, *stdout_nxo;
	cw_nxo_threade_t	error;
	cw_nxoe_name_t	*name;

	ostack = nxo_thread_ostack_get(a_thread);
	NXO_STACK_GET(depth, ostack, a_thread);
	NXO_STACK_DOWN_GET(nnxo, ostack, a_thread, depth);
	if (nxo_type_get(depth) != NXOT_INTEGER || nxo_type_get(nnxo) !=
	    NXOT_NAME) {
	...
	error = nxo_file_output_n(stdout_nxo, name->len, "[s]", name->str);
	if (error) {
		nxo_thread_error(a_thread, error);
		return;
	}
