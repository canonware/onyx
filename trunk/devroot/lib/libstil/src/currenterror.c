...
void
currenterror_l_populate(cw_stilo_t *a_dict, cw_stilo_t *a_thread)
{
	cw_stilo_t	*tstack;
	cw_stilo_t	*name, *val;
...
#define NENTRIES	10	/* Number of entries in currenterror. */
	stilo_dict_new(a_dict, stilo_thread_stil_get(a_thread),
	    stilo_thread_currentlocking(a_thread), NENTRIES);
	...
	stilo_string_new(val, stilo_thread_stil_get(a_thread),
	...
	stilo_name_new(name, stilo_thread_stil_get(a_thread),
	    stiln_str(STILN_line), stiln_len(STILN_line), TRUE);
	stilo_integer_new(val, 1);
	stilo_dict_def(a_dict, stilo_thread_stil_get(a_thread), name, val);
	
	stilo_name_new(name, stilo_thread_stil_get(a_thread),
	    stiln_str(STILN_column), stiln_len(STILN_column), TRUE);
	stilo_integer_new(val, 1);
	stilo_dict_def(a_dict, stilo_thread_stil_get(a_thread), name, val);
