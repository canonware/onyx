...
struct cw_stilo_s
{
   ...
   * Each stilt maintains a cache of stiln pointers, each holding a single
   * reference to the names hash in stil.  If this is an indirect reference, the
   * unreferencing operation should actually be done with the stilt's stiln
   * cache.  Note that this is the (very) common case.  Use the stiln pointer to
   * get the actual name "value" for the unreferencing operation.  This is safe,
   * because this stilt is guaranteed to be holding a reference to the stiln. */
   ...
   *
   * 1 == One reference.
   * 0 == Overflow (GC knows about the stiloe). */
  cw_uint32_t ref_count : 1;
  /* Reference to a local or global object? */
  cw_bool_t global : 1;
  /* This bit is used to protect modifications to a stiloe pointer.  Since a
