	...
	for (sdepth = cdepth = nxo_stack_count(&thread->estack);
		...
		case NXOT_FILE: {
			...
			for (nread = nxo_file_read(nxo,
			    CW_LIBONYX_FILE_EVAL_READ_SIZE, buffer); nread > 0;
			    nread = nxo_file_read(nxo,
			    CW_LIBONYX_FILE_EVAL_READ_SIZE, buffer)) {
