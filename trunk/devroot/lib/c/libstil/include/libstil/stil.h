...
struct cw_stil_s
{
  ...
  /* Hash of names ((cw_stilnk_t *) string is hashed) to (cw_stiln_t *).  This
   * hash table keeps track of *all* name "values" in the virtual machine.  When
   * a name object is created, it actually adds a reference to a stiln and uses
   * a pointer to that stiln as a unique key.  Note that each stilt maintains a
   * cache of stiln's, so that under normal circumstances, all objects in a
   * stilt share a single reference. */
  cw_dch_t stiln_dch;
...
struct cw_stiln_s
{
   ...
   * deallocated during stiln destruction. */
