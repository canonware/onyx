...
struct cw_stil_s {
	 ...
	 * virtual machine.  When a name object is created, it actually adds
	 * a reference to a stiln and uses a pointer to that stiln as a
	 * unique key.  Note that each stilt maintains a cache of stiln's,
	 * so that under normal circumstances, all objects in a stilt share
	 * a single reference.
	 */
	cw_dch_t stiln_dch;
...
struct cw_stilnk_s {
	/*
	 * If a name that is long enough to be stored as a buf is
	 * referenced, the value of the buf is copied to a contiguous
	 * string, so that the representation of the name is always a
	 * contiguous string.  In practice, it would be ridiculous to have
	 * such a long name, but we do the right thing in any case.
	 */
...
struct cw_stiln_s {
	 ...
	 * thread to determine whether an entry exists in a particular
	 * global dictionary without having to lock the entire dictionary.
	 */
	cw_dch_t *keyed_refs;

	/*
	 * If TRUE, the string in the key is statically allocated, and
	 * should not be deallocated during stiln destruction.
	 */
