...
struct cw_stilso_s
{
   ...
   * between (cw_stilso_t *) and (cw_stilo_t *). */
  cw_stilo_t stilo;
   ...
   * stils-wide spares ring.  If this stilso is empty/invalid, the least
   * significant bit of the data pointer is 1.  This allows the GC to deallocate
   * completely unused stilsc's by iteratively unlinking all stilso's in an
   * empty stilsc. */
  cw_ring_t link;
};
...
struct cw_stilsc_s
{
  ...
  /* Linkage into a ring of stilsc's in the same stils.  The GC uses this ring
   * to iterate through the stils, and potentially deallocate empty stilsc's.
   * */
  cw_ring_t link;
  ...
  /* Must be last field, since it is used for array indexing of stilso's beyond
