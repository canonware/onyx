	       ...
	       const void * a_data)
{
  ...
  /* Find the name. */
  if (FALSE == dch_search(&a_stil->stiln_dch, &search_key, (void **) &data))
  {
    /* Add a keyed reference if necessary. */
    if (NULL != a_key)
    {
      mtx_lock(&data->lock);
      if (TRUE == stil_p_stiln_kref(a_stil, data, a_key, a_data))
      {
  ...
  else if (TRUE == a_force)
  {
    ...
    /* Add a keyed reference if necessary.  We don't need to lock the stiln,
		  ...
		  const void * a_data)
{
  ...
  /* Assume that a_stiln is locked, or that no other threads can get to it. */
