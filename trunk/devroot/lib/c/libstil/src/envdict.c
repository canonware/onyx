...
envdict_populate(cw_stilo_t *a_dict, cw_stilt_t *a_stilt, char **a_envp)
{
	...
	/*
	 * Iterate through key/value strings in the environment vector and
	...
	for (i = 0; a_envp[i] != NULL; i++) {
		...
		/* Create key. */
		stilo_name_new(&key_stilo, stilt_stil_get(a_stilt), key_str,
		    key_len, FALSE);

		/* Create value. */
		stilo_string_new(&val_stilo, stilt_stil_get(a_stilt), val_len);
