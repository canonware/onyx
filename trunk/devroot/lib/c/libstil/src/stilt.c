...
stilt_detach_buf(cw_stilt_t * a_stilt, cw_buf_t * a_buf)
{
  ...
  entry_arg
    = (struct cw_stilt_entry_s *) _cw_malloc(sizeof(struct cw_stilt_entry_s));
  if (NULL == entry_arg)
  {
    goto OOM_1;
  }
...
#undef _CW_STILS_PSTATE
     ...
     * having to duplicate the _CW_STILT_STATE_START code. */
    RESTART:
	...
	switch (c)
	{
	  ...
	  case '+':
	  {
	    ...
	    a_stilt->meta.number.sign = _CW_STILS_NUMBER_POS;
	  ...
	  {
	    a_stilt->state = _CW_STILT_STATE_NUMBER;
	    a_stilt->meta.number.sign = _CW_STILS_NUMBER_NEG;
	    a_stilt->meta.number.base = 10;
	    a_stilt->meta.number.point_offset = -1;
	    a_stilt->meta.number.begin_offset = 1;
	    _CW_STILT_PUTC(c);
	...
	switch (c)
	{
	  case '.':
	  {
	    ...
	    {
	      a_stilt->meta.number.point_offset = a_stilt->index;
	    }
	    else
	    {
	      a_stilt->state = _CW_STILT_STATE_NAME;
	  ...
	  case '#':
	  {
	    ...
	    ndigits = a_stilt->index - a_stilt->meta.number.begin_offset;
	    
	    if ((a_stilt->meta.number.point_offset != -1)
		|| (a_stilt->meta.number.begin_offset == a_stilt->index))
	    {
	    ...
	    else
	    {
	      ...
	      for (i = 0; i < ndigits; i++)
	      {
		digit = _CW_STILT_GETC(a_stilt->meta.number.begin_offset + i)
			 - '0';

		if (a_stilt->index - a_stilt->meta.number.begin_offset - i == 2)
		{
		  digit *= 10;
		}
		a_stilt->meta.number.base += digit;
		...
		a_stilt->meta.name.is_immediate = FALSE;
	      ...
	      {
		a_stilt->meta.number.begin_offset = a_stilt->index + 1;
	      }
	    }
	  ...
	  case '{': case '}': case '/': case '%':
	  {
	    /* New token. */
	    a_stilt->state = _CW_STILT_STATE_START;
	    if ((1 < (a_stilt->index - a_stilt->meta.number.begin_offset))
		|| ((0 < (a_stilt->index - a_stilt->meta.number.begin_offset))
		    && (-1 == a_stilt->meta.number.point_offset)))
	    {
	      stilt_p_print_token(a_stilt, a_stilt->index, "number");
	    }
	    else
	    {
	...
	switch (c)
	{
	  case '\\':
	  {
	    a_stilt->state = _CW_STILT_STATE_ASCII_STRING_PROT_CONT;
	    break;
	  }
	  case '(':
	  {
	  ...
	  case ')':
	  {
	    ...
	    if (0 == a_stilt->meta.string.paren_depth)
	    {
	      ...
	      a_stilt->state = _CW_STILT_STATE_START;
	      stilt_p_print_token(a_stilt, a_stilt->index, "string");
	      stilt_p_reset_tok_buffer(a_stilt);
	    }
	    else
	    {
	  ...
	  case '\r':
	  {
	    a_stilt->state = _CW_STILT_STATE_ASCII_STRING_NEWLINE_CONT;
	    break;
	  }
	  default:
	  {
	...
	switch (c)
	{
	  ...
	  case 'x':
	  {
	    a_stilt->state = _CW_STILT_STATE_ASCII_STRING_HEX_CONT;
	    break;
	  }
	  case '\r':
	  {
	    a_stilt->state = _CW_STILT_STATE_ASCII_STRING_CRLF_CONT;
	    break;
	  }
	  case '\n':
	  {
	...
	switch (c)
	{
	  ...
	  case 'c': case 'd': case 'e': case 'f':
	  {
	    a_stilt->state = _CW_STILT_STATE_ASCII_STRING_HEX_FINISH;
	...
	switch (c)
	{
	  ...
	  case 'c': case 'd': case 'e': case 'f':
	  {
	    ...
	    switch (a_stilt->meta.string.hex_val)
	    {
	      ...
	      case '6': case '7': case '8': case '9':
	      {
		val = (a_stilt->meta.string.hex_val - '0') << 4;
		break;
	      }
	      case 'a': case 'b': case 'c': case 'd': case 'e': case 'f':
	      {
		val = (a_stilt->meta.string.hex_val - 'a') << 4;
		break;
	      }
	      default:
	      {
	...
	switch (c)
	{
	  ...
	  {
	    a_stilt->state = _CW_STILT_STATE_START;
	    stilt_p_print_token(a_stilt, a_stilt->index, "hex string");
	    stilt_p_reset_tok_buffer(a_stilt);
	    break;
	  }
	  case 'A': case 'B': case 'C': case 'D': case 'E': case 'F':
	...
	switch (c)
	{
	  ...
	  {
	    a_stilt->state = _CW_STILT_STATE_START;
	    stilt_p_print_token(a_stilt, a_stilt->index, "base 85 string");
	    stilt_p_reset_tok_buffer(a_stilt);
	    break;
	  }
	  default:
...
stilt_p_putc(cw_stilt_t * a_stilt, cw_uint32_t a_c)
{
...
#endif
  {
    if (_CW_STIL_BUFC_SIZE == a_stilt->index)
    {
      ...
      memcpy(kbufc->buffer, a_stilt->tok_buffer.str, _CW_STIL_BUFC_SIZE);
      buf_new(&a_stilt->tok_buffer.buf);
      if (TRUE == buf_append_bufc(&a_stilt->tok_buffer.buf, &kbufc->bufc,
				  0, _CW_STIL_BUFC_SIZE))
      {
...
stilt_p_print_syntax_error(cw_stilt_t * a_stilt, cw_uint8_t a_c)
{
  ...
  if (_CW_STIL_BUFC_SIZE >= a_stilt->index)
  {
    out_put_n(cw_g_out, a_stilt->index, "[s]", a_stilt->tok_buffer.str);
  }
  else
  {
    out_put_n(cw_g_out, a_stilt->index, "[b]", &a_stilt->tok_buffer.buf);
  }
  out_put(cw_g_out, "<--\n");
  a_stilt->state = _CW_STILT_STATE_START;
  stilt_p_reset_tok_buffer(a_stilt);
