...
main()
{
	...
	/* dch_new(), dch_delete(). */
	{
		...
		dch_a = dch_new(NULL, 2, 2, 1, NULL, ch_hash_string,
		    ch_key_comp_string);
		_cw_check_ptr(dch_a);
		dch_delete(dch_a);

		_cw_assert(&dch_b == dch_new(&dch_b, 4, 3, 1, NULL,
	...
	/* dch_count(), dch_insert(). */
	{
		...
		dch = dch_new(NULL, 4, 2, 1, NULL, ch_hash_string,
		    ch_key_comp_string);
		_cw_check_ptr(dch);
	...
	/* dch_count, dch_remove(). */
	{
		...
		_cw_assert(FALSE == dch_remove(dch, a, (void **)&k, (void
		    **)&v));
		_cw_assert(a == k);
		_cw_assert(a == v);
		_cw_assert(3 == dch_count(dch));
		...
		_cw_assert(FALSE == dch_remove(dch, c, (void **)&k, (void
		    **)&v));
		_cw_assert(d == k);
		_cw_assert(d == v);
		_cw_assert(1 == dch_count(dch));
	...
	/* dch_search(). */
	{
		...
		dch = dch_new(NULL, 4, 2, 1, chi_pezz, ch_hash_string,
		    ch_key_comp_string);
		_cw_check_ptr(dch);
	...
	/* dch_get_iterate(), dch_remove_iterate(). */
	{
		...
		dch = dch_new(NULL, 3, 2, 1, chi_pezz, ch_hash_string,
		    ch_key_comp_string);
		_cw_check_ptr(dch);
		...
		_cw_assert(FALSE == dch_get_iterate(dch, (void **)&k, (void
		    **)&v));
		_cw_assert(a == k);
		_cw_assert(a == v);
		...
		_cw_assert(FALSE == dch_remove_iterate(dch, (void **)&k, (void
		    **)&v));
		_cw_assert(b == k);
		_cw_assert(b == v);
		...
		_cw_assert(TRUE == dch_remove_iterate(dch, (void **)&k, (void
		    **)&v));
		_cw_assert(TRUE == dch_get_iterate(dch, (void **)&k, (void
		    **)&v));
