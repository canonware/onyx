...
main()
{
	...
	/* Create the data. */
	{
		...
		for (i = 0; i < 4; i++) {
			bufc = bufc_new((cw_bufc_t
			    *)_cw_malloc(sizeof(cw_bufc_t)), mem_dealloc,
			    cw_g_mem);
			data = _cw_malloc(4096);
			bufc_set_buffer(bufc, data, 4096, FALSE, mem_dealloc,
			    cw_g_mem);
		...
		for (i = 0; i < 1024; i++) {
			bufc = bufc_new((cw_bufc_t
			    *)_cw_malloc(sizeof(cw_bufc_t)), mem_dealloc,
			    cw_g_mem);
			data = _cw_malloc(16);
			bufc_set_buffer(bufc, data, 16, FALSE, mem_dealloc,
			    cw_g_mem);
