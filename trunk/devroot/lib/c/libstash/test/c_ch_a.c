...
main()
{
	...
	/* ch_new(), ch_delete(). */
	{
		...
		ch_a = ch_new(NULL, 2, NULL, ch_hash_string,
		    ch_key_comp_string);
		_cw_check_ptr(ch_a);
		ch_delete(ch_a);
	...
	/* ch_search(). */
	{
		...
		ch = ch_new(NULL, 4, chi_pezz, ch_hash_string,
		    ch_key_comp_string);
		_cw_check_ptr(ch);
	...
	/* ch_get_iterate(), ch_remove_iterate(). */
	{
		...
		_cw_assert(FALSE == ch_get_iterate(ch, (void **)&k, (void
		    **)&v));
		_cw_assert(a == k);
		_cw_assert(a == v);
		...
		_cw_assert(FALSE == ch_remove_iterate(ch, (void **)&k, (void
		    **)&v));
		_cw_assert(b == k);
		_cw_assert(b == v);
		...
		_cw_assert(TRUE == ch_remove_iterate(ch, (void **)&k, (void
		    **)&v));
		_cw_assert(TRUE == ch_get_iterate(ch, (void **)&k, (void
		    **)&v));
