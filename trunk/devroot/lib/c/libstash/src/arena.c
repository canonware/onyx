...
arena_delete(cw_arena_t *a_arena)
{
	...
	for (p = list_get_next(&a_arena->chunks, NULL); NULL != p; p =
	    list_get_next(&a_arena->chunks, p)) {
    ...
    const char *a_filename, cw_uint32_t a_line_num)
{
	...
	/*
	 * Now adjust next_alloc_location to point to the next doubleword
	 * boundary past the current allocation.
	 */
#define INCR_TO_NEXT_ALIGN(x, n)					\
	...
	/*
	 * If the next allocation location is past the end of this chunk, we
	 * need a new chunk.
	 */
	if (a_arena->chunk_size < new_alloc_location) {
		...
		if (NULL == new_chunk) {
			 ...
			 * We're full now.
			 *
			 * Don't set retval to NULL here, since the current
			 * allocation succeeded, but set the
			 * next_alloc_location to new_alloc_location so all
		...
		if (NULL == list_hpush(&a_arena->chunks, new_chunk)) {
			/*
			 * Memory allocation error.  Don't set retval to
			 * NULL here, since the current allocation
			 * succeeded, but set the next_alloc_location to
			 * new_alloc_location so all further allocations
			 * will fail.
			 */
			_cw_free(new_chunk);
			goto RETURN;
