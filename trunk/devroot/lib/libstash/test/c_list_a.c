...
main()
{
	...
	/*
	 * list_hpush(), list_hpop, list_hpeek(), list_count().
	 */
	{
		cw_list_t *list;
		cw_sint32_t ints[13], i;
	...
	/*
	 * list_tpush(), list_tpop(), list_tpeek(), list_purge_spares(),
	 * list_count().
	 */
	{
		cw_list_t *list;
	 ...
	 */
	{
		...
		for (i = 0; i < 11; i++) {
			...
			_cw_assert(NULL != list_insert_before(list, items[i],
			    &ints_b[i]));
			_cw_assert((i + 12) == list_count(list));
		}

		/*
		 * Push and pop from each end of the list to make sure the
		 * head and tail are still okay.  If they aren't, the
		 * problem will show up later.
		 */
		list_hpush(list, list_hpop(list));
		list_tpush(list, list_tpop(list));

		/*
		 * Make sure that things are in the proper order in the
		 * list.
		 */
		for (i = 0; i < 11; i++) {
	...
	/*
	 * list_insert_after(), list_purge_spares(), list_count().
	 */
	{
		cw_list_t *list;
		cw_list_item_t *items[11];
		...
		for (i = 0; i < 11; i++) {
			ints_b[i] = i + 11;
			_cw_assert((i + 11) == list_count(list));
			_cw_assert(NULL != list_insert_after(list, items[i],
			    &ints_b[i]));
			_cw_assert((i + 12) == list_count(list));
		}
	...
	/*
	 * list_remove_container(), list_count().
	 */
	{
		cw_list_t *list;
		cw_list_item_t *items[11];
		...
		for (i = 0; i < 11; i++) {
			...
			_cw_assert(ints[i] ==
			    *(cw_uint32_t *)list_remove_container(list,
			        items[i]));
		}
...
#define NUM_ITEMS 50
	{
		...
		for (i = 0; i < NUM_ITEMS; i++) {
			...
			_cw_out_put_s(strings[i], "This is string [i]", i);
		}
		...
		_cw_out_put(
		    "hpop()ping from list1 and hpush()ing to list2\n");
		...
		_cw_out_put(
		    "tpop()ping from list2 and hpush()ing to list2\n");
		...
		_cw_assert(list_count(list1) == 0);
		_cw_out_put("list2->count == [q]\n", list_count(&list2));

		_cw_assert(list_count(&list2) == NUM_ITEMS);

		_cw_out_put(
		    "tpop()ping from list2 and tpush()ing to list1\n");
