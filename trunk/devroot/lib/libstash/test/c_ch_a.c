...
main()
{
	...
	/* ch_new(), ch_delete(). */
	{
		...
		ch_a = ch_new(NULL, cw_g_mem, 2, ch_hash_string,
		    ch_key_comp_string);
		_cw_check_ptr(ch_a);
		ch_delete(ch_a);

		_cw_assert(ch_new(&ch_b, cw_g_mem, 1, ch_hash_direct,
		    ch_key_comp_direct) == &ch_b);
		ch_delete(&ch_b);
	}
	...
	/* ch_count(), ch_insert(). */
	{
		...
		ch = ch_new(NULL, cw_g_mem, 4, ch_hash_string,
		    ch_key_comp_string);
		_cw_check_ptr(ch);
