...
#endif
static void	buf_p_writeable_range_make(cw_buf_t *a_buf, cw_uint32_t
...
/*  		    sizeof(cw_bufel_t)); */
	...
	if (a_buf->bufel_array != a_buf->static_bufel_array)
		mem_free(a_buf->mem, a_buf->bufel_array);
	if (a_buf->cumulative_index != a_buf->static_cumulative_index)
		mem_free(a_buf->mem, a_buf->cumulative_index);
	if (a_buf->iov != a_buf->static_iov)
		mem_free(a_buf->mem, a_buf->iov);
	if (a_buf->is_malloced)
		mem_free(a_buf->mem, a_buf);
}
...
buf_dump(cw_buf_t *a_buf, const char *a_prefix)
{
	...
	out_put(out_err, "[s]| buf_dump()\n", a_prefix);
#ifdef _CW_DBG
	out_put(out_err, "[s]|--> magic_a : 0x[i|b:16]\n", a_prefix,
	    a_buf->magic_a);
	out_put(out_err, "[s]|--> magic_b : 0x[i|b:16]\n", a_prefix,
	    a_buf->magic_b);
	out_put(out_err, "[s]|--> size_of : [i]\n", a_prefix, a_buf->size_of);
#endif
	out_put(out_err, "[s]|--> is_malloced : [s]\n", a_prefix,
	    (a_buf->is_malloced) ? "TRUE" : "FALSE");
	out_put(out_err, "[s]|--> is_threadsafe : [s]\n", a_prefix,
	    (a_buf->is_threadsafe) ? "TRUE" : "FALSE");
	out_put(out_err, "[s]|--> size : [i]\n", a_prefix, a_buf->size);
	out_put(out_err, "[s]|--> array_size : [i]\n", a_prefix,
	    a_buf->array_size);
	out_put(out_err, "[s]|--> array_num_valid : [i]\n", a_prefix,
	    a_buf->array_num_valid);
	out_put(out_err, "[s]|--> array_start : [i]\n", a_prefix,
	    a_buf->array_start);
	out_put(out_err, "[s]|--> array_end : [i]\n", a_prefix,
	    a_buf->array_end);
	out_put(out_err, "[s]|--> is_cumulative_valid : [s]\n",
	    a_prefix, (a_buf->is_cumulative_valid) ? "TRUE" : "FALSE");
	out_put(out_err, "[s]|--> is_cached_bufel_valid : [s]\n", a_prefix,
	    (a_buf->is_cached_bufel_valid) ? "TRUE" : "FALSE");
	out_put(out_err, "[s]|--> cached_bufel : [i]\n", a_prefix,
	    a_buf->cached_bufel);
	...
	for (i = 0; i < a_buf->array_size; i++) {
...
#endif
		out_put(out_err, "[s]|   |--> beg_offset : [i]\n", a_prefix,
		    a_buf->bufel_array[i].beg_offset);
		out_put(out_err, "[s]|   |--> end_offset : [i]\n", a_prefix,
...
#endif
		{
			...
			out_put(out_err, "[s]|   |--> bufc : 0x[i|b:16]\n"
			    "[s]|    \\\n", a_prefix,
			    a_buf->bufel_array[i].bufc, a_prefix);

			sub_prefix = mem_malloc(a_buf->mem, strlen(a_prefix)
			    + 7);
			if (sub_prefix == NULL)
...
buf_buf_catenate(cw_buf_t *a_a, cw_buf_t *a_b, cw_bool_t a_preserve)
{
	...
	buf_p_buf_catenate(a_a, a_b, a_preserve);
...
buf_split(cw_buf_t *a_a, cw_buf_t *a_b, cw_uint32_t a_offset)
{
	...
	if ((a_offset > 0) && (a_offset < a_b->size)) {
		 ...
		 */
		buf_p_array_fit(a_a, a_a->array_num_valid + num_bufels_to_move);
...
/*  			a_b->cached_bufel = 0; */
		...
		/* Same as catenation. */
		buf_p_buf_catenate(a_a, a_b, FALSE);
	}
...
#define _CW_BUF_OR_BYTE(o, s)					\
	...
	if (bufel_offset + 3 <
		...
		retval = _CW_BUF_OR_BYTE(0, 3);
		retval |= _CW_BUF_OR_BYTE(1, 2);
		retval |= _CW_BUF_OR_BYTE(2, 1);
		retval |= _CW_BUF_OR_BYTE(3, 0);
#else
		retval = _CW_BUF_OR_BYTE(0, 0);
		retval |= _CW_BUF_OR_BYTE(1, 1);
		retval |= _CW_BUF_OR_BYTE(2, 2);
		retval |= _CW_BUF_OR_BYTE(3, 3);
#endif
	} else {
#ifdef WORDS_BIGENDIAN
		retval = _CW_BUF_OR_BYTE(0, 3);
...
#define _CW_BUF_OR_BYTE(o, s)					\
	...
	if (bufel_offset + 7 < a_buf->bufel_array[array_element].end_offset) {
#ifdef WORDS_BIGENDIAN
		retval = _CW_BUF_OR_BYTE(0, 7);
		retval |= _CW_BUF_OR_BYTE(1, 6);
		retval |= _CW_BUF_OR_BYTE(2, 5);
		retval |= _CW_BUF_OR_BYTE(3, 4);
		retval |= _CW_BUF_OR_BYTE(4, 3);
		retval |= _CW_BUF_OR_BYTE(5, 2);
		retval |= _CW_BUF_OR_BYTE(6, 1);
		retval |= _CW_BUF_OR_BYTE(7, 0);
#else
		retval = _CW_BUF_OR_BYTE(0, 0);
		retval |= _CW_BUF_OR_BYTE(1, 1);
		retval |= _CW_BUF_OR_BYTE(2, 2);
		retval |= _CW_BUF_OR_BYTE(3, 3);
		retval |= _CW_BUF_OR_BYTE(4, 4);
		retval |= _CW_BUF_OR_BYTE(5, 5);
		retval |= _CW_BUF_OR_BYTE(6, 6);
		retval |= _CW_BUF_OR_BYTE(7, 7);
#endif
	} else {
#ifdef WORDS_BIGENDIAN
		retval = _CW_BUF_OR_BYTE(0, 7);
		buf_p_data_position_get(a_buf, a_offset + 1, &array_element,
		    &bufel_offset);
		retval |= _CW_BUF_OR_BYTE(0, 6);
		buf_p_data_position_get(a_buf, a_offset + 2, &array_element,
		    &bufel_offset);
		retval |= _CW_BUF_OR_BYTE(0, 5);
		buf_p_data_position_get(a_buf, a_offset + 3, &array_element,
		    &bufel_offset);
		retval |= _CW_BUF_OR_BYTE(0, 4);
...
buf_uint8_set(cw_buf_t *a_buf, cw_uint32_t a_offset, cw_uint8_t a_val)
{
	...
	buf_p_writeable_range_make(a_buf, a_offset, sizeof(cw_uint8_t));
	buf_p_data_position_get(a_buf, a_offset, &array_element, &bufel_offset);
...
buf_uint32_set(cw_buf_t *a_buf, cw_uint32_t a_offset, cw_uint32_t a_val)
{
	...
	buf_p_writeable_range_make(a_buf, a_offset, sizeof(cw_uint32_t));
	/* Get a byte from a_val and put in in the least significant byte. */
#define _CW_BUF_BYTE(s) ((a_val >> ((s) << 3)) & 0xff)
...
buf_uint64_set(cw_buf_t *a_buf, cw_uint32_t a_offset, cw_uint64_t a_val)
{
	...
	buf_p_writeable_range_make(a_buf, a_offset, sizeof(cw_uint64_t));
	/* Get a byte from a_val and put in in the least significant byte. */
#define _CW_BUF_BYTE(s) ((a_val >> ((s) << 3)) & 0xff)
...
/*  		    sizeof(cw_bufel_t)); */
		...
		buf_p_writeable_range_make(a_buf, a_offset, a_length);
...
buf_p_new(cw_buf_t *a_buf, cw_mem_t *a_mem, cw_bool_t a_is_threadsafe)
{
	...
	if (a_buf == NULL) {
		retval = (cw_buf_t *)mem_malloc(a_mem, sizeof(cw_buf_t));
	...
	retval->array_size = _CW_BUF_ARRAY_MIN_SIZE;
...
/*  	retval->cached_bufel = 0; */
	...
	memset(retval->bufel_array, 0,
	    _CW_BUF_ARRAY_MIN_SIZE * sizeof(cw_bufel_t));
	memset(retval->cumulative_index, 0,
	    _CW_BUF_ARRAY_MIN_SIZE * sizeof(cw_uint32_t));
	memset(retval->iov, 0,
	    _CW_BUF_ARRAY_MIN_SIZE * sizeof(struct iovec));
#endif
...
buf_p_array_fit(cw_buf_t *a_buf, cw_uint32_t a_min_array_size)
{
	...
	if (a_min_array_size > a_buf->array_size) {
		...
		if (a_buf->bufel_array != a_buf->static_bufel_array) {
			t_ptr = mem_realloc(a_buf->mem, a_buf->bufel_array,
			    i * sizeof(cw_bufel_t));
		} else {
			t_ptr = mem_calloc(a_buf->mem, i,
			    sizeof(cw_bufel_t));
		...
		if (a_buf->iov != a_buf->static_iov) {
			t_ptr = mem_realloc(a_buf->mem, a_buf->iov, i *
			    sizeof(struct iovec));
		} else {
			t_ptr = mem_calloc(a_buf->mem, i, sizeof(struct
			    iovec));
#ifdef _CW_DBG
...
buf_p_buf_catenate(cw_buf_t *a_a, cw_buf_t *a_b, cw_bool_t a_preserve)
{
	...
	buf_p_array_fit(a_a, a_a->array_num_valid + a_b->array_num_valid);
	/*
    ...
    a_length)
{
	...
	volatile cw_uint32_t	i, num_iterations;
	cw_bufel_t		*bufel;
	cw_bufc_t		*bufc;
	volatile cw_bufc_t	*v_bufc;
	void			*buffer;
	...
	if (a_offset + a_length > a_buf->size) {
		...
		xep_begin();
		xep_try {
			v_bufc = bufc = bufc_new(NULL, a_buf->mem,
			    (cw_opaque_dealloc_t *)mem_free_e, cw_g_mem);
			try_stage = 1;

			buffer = mem_malloc(a_buf->mem, (a_offset +
			    a_length) - a_buf->size);
			try_stage = 2;
		}
		xep_catch(_CW_STASHX_OOM) {
		...
		bufc_buffer_set(bufc, buffer, (a_offset + a_length) -
		    a_buf->size, TRUE, (cw_opaque_dealloc_t *)mem_free_e,
		    cw_g_mem);
...
/*  		    sizeof(cw_bufel_t)); */
	...
	for (i = 0; i < num_iterations; i++) {
		...
		if ((bufc_p_is_writeable(bufel->bufc) == FALSE) ||
			...
			buffer = mem_malloc(a_buf->mem, bufel->end_offset -
			    bufel->beg_offset);
			xep_begin();
			xep_try {
				bufc = bufc_new(NULL, a_buf->mem, NULL, NULL);
    ...
    *a_dealloc_func, void *a_dealloc_arg)
{
	...
	if (a_bufc == NULL) {
		retval = (cw_bufc_t *)mem_malloc(a_mem,
		    sizeof(cw_bufc_t));
...
bufc_p_dump(cw_bufc_t *a_bufc, const char *a_prefix)
{
...
#endif
	out_put(out_err, "[s]|--> free_func : 0x[p]\n", a_prefix,
	    a_bufc->dealloc_func);
	out_put(out_err, "[s]|--> free_arg : 0x[p]\n", a_prefix,
	    a_bufc->dealloc_arg);
	out_put(out_err, "[s]|--> ref_count : [i]\n", a_prefix,
	    a_bufc->ref_count);
	out_put(out_err, "[s]|--> is_writeable : [s]\n", a_prefix,
	    a_bufc->is_writeable ? "TRUE" : "FALSE");
	out_put(out_err, "[s]|--> buf_size : [i]\n", a_prefix,
	    a_bufc->buf_size);
	out_put(out_err, "[s]\\--> buf (0x[i|w:8|p:0|b:16]) : ", a_prefix,
	    a_bufc->buf);
	...
	for (i = 0; i < a_bufc->buf_size; i++) {
		...
		out_put(out_err, "[i|w:2|p:0|b:16] ", a_bufc->buf[i]);
	}
	out_put(out_err, "\n");
