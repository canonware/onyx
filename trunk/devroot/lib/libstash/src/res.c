...
res_new(cw_res_t *a_res)
{
	...
	/*
	 * Non-thread-safe hash table, since we're already taking care of
	 * the locking.
	 */
	oh_new(&retval->hash);
...
res_delete(cw_res_t *a_res)
{
	...
	for (i = 0, num_resources = oh_get_num_items(&a_res->hash); i <
	    num_resources; i++) {
...
res_is_equal(cw_res_t *a_a, cw_res_t *a_b)
{
	...
	if (a_a == a_b) {
		...
		for (i = 0, retval = FALSE; (i < num_resources) && (retval ==
		    FALSE); i++) {
			oh_item_delete_iterate(&a_a->hash, (void **)&key, (void
			    **)&val);
...
res_get_res_val(cw_res_t *a_res, const char *a_res_name)
{
	...
	error = oh_item_search(&a_res->hash, (void *)a_res_name, (void
	    **)&retval);
	if (error == TRUE)
		retval = NULL;
    ...
    char **r_res_name, char **r_res_val)
{
	...
	retval = oh_item_delete(&a_res->hash, a_res_key, (void **)r_res_name,
	    (void **)r_res_val);
...
res_p_parse_res(cw_res_t *a_res, cw_bool_t a_is_file)
{
	...
	for (i = 0, col_num = 1; ((state != _LIBSTASH_RES_STATE_FINISH) &&
		...
		/*
		 * Check whether we overflowed the buffers, and expand them,
		 * if necessary.
		 */
		...
		case _LIBSTASH_RES_STATE_START:
			...
			col_num = 1;
			/*
			 * Truncate.  Not strictly necessary with
			 * static buffers.
			 */
			/* name = '\0'; */
			/* val = '\0'; */
		...
		case _LIBSTASH_RES_STATE_BEGIN_WHITESPACE:
			...
			case _LIBSTASH_RES_CHAR_WHITESPACE:
				/*
				 * More whitespace.  Swallow
				 * it.
				 */
				break;
			case _LIBSTASH_RES_CHAR_NEWLINE:
				/*
				 * Blank line.  Jump back to
				 * the start state to make
				 * sure all counters are
				 * correctly reset.
				 */
				line_num++;
				col_num = 1;
			...
			break;
		/*
		 * Comment that was preceded only by whitespace
		 * characters.
		 */
		...
		case _LIBSTASH_RES_STATE_NAME:
			...
			case _LIBSTASH_RES_CHAR_WHITESPACE:
				/*
				 * End of name.  Jump to a
				 * state that can deal with
			...
			case _LIBSTASH_RES_CHAR_COLON:
				/*
				 * Okay, here's the colon.
				 * Terminate the name and
				 * jump to a state that can
		...
		case _LIBSTASH_RES_STATE_POST_NAME_WHITESPACE:
			...
			case _LIBSTASH_RES_CHAR_COLON:
				/*
				 * Here's the colon.  Jump
				 * to a state that can deal
				 * with whitespace leading
				 * the value.
				 */
				state = _LIBSTASH_RES_STATE_POST_COLON_WHITESPACE;
				break;
		...
		case _LIBSTASH_RES_STATE_POST_COLON_WHITESPACE:
			...
			case _LIBSTASH_RES_CHAR_BACKSLASH:
				/*
				 * Beginning of value, but
				 * it's a backslash, so jump
				 * to the backslash handling
				 * state.
				 */
		...
		case _LIBSTASH_RES_STATE_VALUE:
			...
			case _LIBSTASH_RES_CHAR_PERIOD:
			case _LIBSTASH_RES_CHAR_COLON:
			case _LIBSTASH_RES_CHAR_WHITESPACE:	/* Allow whitespace in
								 * value. */
			case _LIBSTASH_RES_CHAR_VALID_IN_VAL:
				/* More of the value. */
				val[val_pos] = c;
			...
			case _LIBSTASH_RES_CHAR_HASH:
				/*
				 * Beginning of comment, and
				 * therefore the end of the
				 * value. NULL-terminate the
				 * string, insert the
				 * resource, and jump to the
			...
			case _LIBSTASH_RES_CHAR_NEWLINE:
				 ...
				 * value. NULL-terminate the
				 * string, insert the
				 * resource in the hash
				 * table, and jump back to
				 * the starting state.
				 */
				line_num++;
				col_num = 1;
			...
			case _LIBSTASH_RES_CHAR_NULL:
				/*
				 * Do the same thing as for
				 * a newline, except that we
				 * want the state machine to
				 * exit.
				 */
				val[val_pos] = '\0';
				if (res_p_merge_res(a_res, name, val)) {
		...
		case _LIBSTASH_RES_STATE_VALUE_BACKSLASH:
			...
			case _LIBSTASH_RES_CHAR_BACKSLASH:
				/*
				 * Insert a backslash and
				 * jump back to the value
				 * state.
				 */
				val[val_pos] = '\\';
				val_pos++;
			...
			case _LIBSTASH_RES_CHAR_HASH:
				/*
				 * Insert a hash and jump
				 * back to the value state.
				 */
				val[val_pos] = '#';
				val_pos++;
				state = _LIBSTASH_RES_STATE_VALUE;
			...
			case _LIBSTASH_RES_CHAR_WHITESPACE:
				 ...
				 * newline, but we can't do
				 * that if we stay in this
				 * state, since we woudn't
				 * notice '\\' and '#' later
				 * on in the stream.
				 */
			...
			case _LIBSTASH_RES_CHAR_NEWLINE:
				/*
				 * \ continuation.  Swallow
				 * this and jump back to the
				 * value state.
				 */
				line_num++;
				col_num = 1;
			...
			case _LIBSTASH_RES_CHAR_LOWER:
				...
				/*
				 * Note that if it's not an
				 * 'n', we fall through to
				 * the error case.
				 */
			case _LIBSTASH_RES_CHAR_CAP:
			case _LIBSTASH_RES_CHAR_NUMBER:
