...
oh_delete(cw_oh_t *a_oh)
{
	...
	/* Delete the items in the table, as well as items_list. */
	{
		...
		for (i = 0; i < a_oh->items_count; i++) {
			...
			ring_delete(&item->ring_item);	/* Not absolutely
							 * necessary. */
			_cw_free(item);
		}
	}
    ...
    cw_uint32_t a_h2)
{
	...
	if (a_oh->is_thread_safe)
		rwl_wlock(&a_oh->rw_lock);
	if (((a_h2 % 2) == 0) || (a_h2 > (1 << a_oh->base_power)))
		retval = TRUE;
	else {
		retval = FALSE;

		a_oh->base_h2 = a_h2;
		a_oh->curr_h2 = (((a_oh->base_h2 + 1) << (a_oh->curr_power -
		    a_oh->base_power)) - 1);
		oh_p_rehash(a_oh);
	}
    ...
    cw_uint32_t a_shrink_point)
{
	...
	if (a_oh->is_thread_safe)
		rwl_wlock(&a_oh->rw_lock);
	if ((a_shrink_point >= a_oh->base_grow_point) || (a_shrink_point >= (1
	    << a_oh->base_power)))
		retval = TRUE;
	else {
		retval = FALSE;
		...
		a_oh->curr_shrink_point = (a_oh->base_shrink_point <<
    ...
    cw_uint32_t a_grow_point)
{
	...
	if (a_oh->is_thread_safe)
		rwl_wlock(&a_oh->rw_lock);
	if ((a_grow_point <= a_oh->base_shrink_point) || (a_grow_point >= (1
	    << a_oh->base_power)))
		retval = TRUE;
	else {
		retval = FALSE;
...
oh_item_insert(cw_oh_t *a_oh, const void *a_key, const void *a_data)
{
	...
	if (oh_p_item_search(a_oh, a_key, &junk) == TRUE) {
		...
		if (0 < a_oh->spares_count) {
			...
			item = (cw_oh_item_t *)
			    _cw_malloc(sizeof(cw_oh_item_t));
			if (NULL == item) {
				retval = -1;
				goto RETURN;
			}
    ...
    void **r_data)
{
	...
	if (oh_p_item_search(a_oh, a_search_key, &slot) == FALSE) {
		...
		/*
		 * Set the return variables, decrement the item count, and
		 * delete the item.
		 */
		if (NULL != r_key)
		...
		if (a_oh->items_ring == &a_oh->items[slot]->ring_item) {
			/*
			 * The items_ring head is the same as the item we're
			 * removing, so take care to set the items_ring head
			 * correctly.
			 */
...
oh_p_grow(cw_oh_t *a_oh)
{
	...
	if (a_oh->items_count >= a_oh->curr_grow_point) {
		...
		a_oh->curr_h2 = (((a_oh->base_h2 + 1) << (a_oh->curr_power -
		    a_oh->base_power)) - 1);
		a_oh->curr_shrink_point = a_oh->curr_shrink_point << 1;
		a_oh->curr_grow_point = a_oh->curr_grow_point << 1;
		 ...
		 */
		{
			...
			for (i = 0, count = a_oh->items_count; i < count; i++) {
				t_ring = a_oh->items_ring;
				a_oh->items_ring = ring_cut(t_ring);
				a_oh->items_count--;
				if (0 == a_oh->items_count)
...
oh_p_shrink(cw_oh_t *a_oh)
{
	...
	/* Should we shrink? */
	if ((a_oh->items_count < a_oh->curr_shrink_point) && (a_oh->curr_power
	    > a_oh->base_power)) {

		for (j = a_oh->curr_power - a_oh->base_power;
		    (((a_oh->curr_grow_point >> j) < a_oh->items_count) && (j >
		    0)); j--);

		num_halvings = j;

		/* We're not shrinking below the base table size, are we? */
		_cw_assert((a_oh->curr_power - num_halvings) >=
		    a_oh->base_power);
		...
		/* Allocate new table */
		a_oh->items = (cw_oh_item_t **) _cw_realloc(a_oh->items,
		    a_oh->size * sizeof(cw_oh_item_t *));

		/*
		 * We're always shrinking, so there should never be an
		 * error.
		 */
		...
		a_oh->curr_h2 = (((a_oh->base_h2 + 1) << (a_oh->curr_power -
		    a_oh->base_power)) - 1);
		a_oh->curr_shrink_point = a_oh->curr_shrink_point >>
		    num_halvings;
		a_oh->curr_grow_point = a_oh->curr_grow_point >> num_halvings;

		/*
		 * Iterate through old table and insert items into new
		 * table.
		 */
		{
			cw_uint64_t i;
		...
		if (a_oh->spares_count > a_oh->curr_grow_point) {
			...
			for (i = 0, num_to_delete = a_oh->spares_count -
			    a_oh->curr_grow_point; i < num_to_delete; i++) {
				t_ring = a_oh->spares_ring;
				a_oh->spares_ring = ring_cut(t_ring);
				a_oh->spares_count--;
				if (0 == a_oh->spares_count)
    ...
    cw_oh_item_t * a_item)
{
	...
	for (i = 0, j = slot; i < a_oh->size; i++, j = (j + a_oh->curr_h2) %
	    a_oh->size) {
    ...
    cw_uint64_t *a_slot)
{
	 ...
	 */
	for (i = 0, j = slot; ; i++, j = (j + a_oh->curr_h2) % a_oh->size) {
		if (a_oh->items[j] == NULL) {
			/*
			 * Hit an empty slot.  What we're looking for isn't
			 * here.
			 */
			retval = TRUE;
			break;
		} else if (TRUE == a_oh->key_compare(a_oh->items[j]->key,
		    a_key)) {
			/* Found it. */
			*a_slot = j;
			retval = FALSE;
...
oh_p_slot_shuffle(cw_oh_t *a_oh, cw_uint64_t a_slot)
{
	...
	for (i = 0, curr_distance = 1, curr_empty = a_slot, curr_look =
	    ((curr_empty + a_oh->curr_h2) % a_oh->size);
	    ((a_oh->items[curr_look] != NULL) && (i < a_oh->size)); i++,
	    curr_distance++, curr_look = (curr_look + a_oh->curr_h2) %
	    a_oh->size) {
		/*
		 * See if this item had to jump at least the current
		 * distance to the last empty slot in this secondary hash
		 * chain.
		 */
		if (a_oh->items[curr_look]->jumps >= curr_distance) {
			/*
			 * This item should be shuffled back to the previous
			 * empty slot.  Do so, and reset curr_distance and
			 * curr_empty.  Also, update the jumps field of the
			 * item we just shuffled, as well as its record of
			 * the slot that it's now in.
			 */
