...
All the standard error handlers except those for interrupt and timeout execute
errordict's stop operator as the last step of handling the error.
...
\hline
stack & {\bf \htmlref{spop}{systemdict:spop}} & stack & Pop an object off
a stack. \\
\hline
...
\hline \hline
[a] [b] & {\bf \htmlref{catenate}{systemdict:token}} & [a b] & \\
(a) (b) & {\bf \htmlref{catenate}{systemdict:token}} & (a b) & \\
`a' `b' & {\bf \htmlref{catenate}{systemdict:token}} & `ab' & \\
\hline
...
\hline
-- & {\bf \htmlref{pwd}{systemdict:dirforeach}} & path & \\
\hline
...
\item[{\stilop{stack}{spop}{stack}}: ]
	...
	\item[Errors(s): ]
		...
		\end{description}
	\item[Description: ]
		Pop an object off of \oparg{stack}.
	\item[Example(s): ]\begin{verbatim}
