...
\item[{\onyxop{operator depth}{operatortype}{--}}: ]
	...
	\item[Example(s): ]
		...
		leading and trailing dashes.  For some operators, it is possible
		for them to print out with three leading and trailing dashes,
		which indicates that the operator is treated specially in the
		interpreter loop to improve performance.  If the interpreter
		cannot determine the name associated with an operator, as will
		be the case for custom operators, the operator will be printed
		as {\tt -operator-}.
		\begin{verbatim}
