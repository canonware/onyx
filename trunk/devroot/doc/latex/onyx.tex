...
share the same backing memory.  When an executable file, hook, name, operator,
or string is interpreted (encountered directly by the interpreter), it is pushed
onto the execution stack and executed; otherwise the object is pushed onto the
operand stack.  When an executable array, file, hook, name, operator, or string
is evaluated, it is pushed onto the execution stack and executed; otherwise the
object is pushed onto the operand stack.  Note that executable arrays are the
...
threshold can be changed or disabled.}
...
\item[{\onyxop{count}{setthreshold}{--}}: ]
	...
	\item[Input(s): ]
		\begin{description}\item[]
		\item[count: ]
			Number of object allocations since the last garbage
...
printed, except for booleans, integers, names, nulls, and strings, which are
always printed syntactically.  If the recursion depth is greater than 0, the
operators will recursively print any contained objects.  All printing is done to
...
globaldict. \\
\hline
-- & {\bf \htmlref{sprintdict}{systemdict:sprintdict}} & dict & Get
sprintdict. \\
\hline
...
\item[{\onyxop{--}{ostack}{stack}}: ]
	...
	\item[Output(s): ]
		\begin{description}\item[]
		\item[stack: ]
			A current snapshot (copy) of the operand stack.
		\end{description}
	\item[Errors(s): ] None.
	\item[Description: ]
		Get a current snapshot of the operand stack.
	\item[Example(s): ]\begin{verbatim}
...
\item[{\onyxop{--}{sprintdict}{dict}}: ]
	...
	\item[Errors(s): ] None.
	\item[Description: ]
		Get sprintdict.  See Section~\ref{sec:sprintdict} for details on
		sprintdict.
	\item[Example(s): ]\begin{verbatim}

onyx:0> sprintdict 0 sprint
-dict-
onyx:0>
		\end{verbatim}
	\end{description}
...
\item[{\onyxop{stack count amount}{sroll}{stack}}: ]
	\begin{description}\item[]
	\item[Input(s): ]
		\begin{description}\item[]
		\item[stack: ]
