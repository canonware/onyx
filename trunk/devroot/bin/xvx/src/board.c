...
board_delete(cw_board_t * a_board)
{
  ...
  _cw_assert(_CW_BOARD_MAGIC == a_board->magic);

  if (TRUE == a_board->is_malloced)
  {
    _cw_free(a_board);
  }
#ifdef _XVX_DEBUG
...
board_dump(cw_board_t * a_board)
{
  ...
  for (i = 0; i < 19; i++)
  {
    out_put(cw_g_out, " [i|w:2]", i);
  }
  out_put(cw_g_out, "\n\n");

  out_put(cw_g_out,
	  "Captured: P1: [i|w:2], P2: [i|w:2], P3: [i|w:2],"
