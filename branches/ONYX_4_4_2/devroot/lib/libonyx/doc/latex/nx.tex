...
\citem{\cfunc[cw\_nxa\_t *]{nx\_nxa\_get}{cw\_nx\_t *a\_nx}}
	...
	\end{capilist}
\label{nx_mema_get}
\index{nx_mema_get@\cfunc{nx\_mema\_get}{}}
\citem{\cfunc[cw\_mema\_t *]{nx\_mema\_get}{cw\_nx\_t *a\_nx}}
	\begin{capilist}
	\item[Input(s): ]
		\begin{description}\item[]
			Pointer to an \classname{nx}.
	...
	\item[Output(s): ]
		\begin{description}\item[]
		\item[retval: ]
			Pointer to an \classname{mema}.
		\end{description}
	\item[Exception(s): ] None.
	\item[Description: ]
		Return a pointer to a memory allocator that encapsulates the
		garbage collector allocator functions.
	\end{capilist}
