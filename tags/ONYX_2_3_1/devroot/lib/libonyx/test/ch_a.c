...
main()
{
	...
	/* ch_new(), ch_delete(). */
	{
		...
		cw_assert(ch_new(&ch_b, (cw_opaque_alloc_t *)mem_malloc_e,
	...
	/* ch_count, ch_remove(). */
	{
		...
		cw_assert(ch_remove(ch, a, (void **)&k, (void **)&v, NULL) ==
		    FALSE);
		cw_assert(k == a);
		cw_assert(v == a);
		cw_assert(ch_count(ch) == 3);
		...
		cw_assert(ch_remove(ch, c, (void **)&k, (void **)&v, NULL) ==
		    FALSE);
		cw_assert(k == d);
		cw_assert(v == d);
		cw_assert(ch_count(ch) == 1);
	...
	/* ch_get_iterate(), ch_remove_iterate(). */
	{
		...
		cw_assert(ch_remove_iterate(ch, (void **)&k, (void **)&v,
		    NULL));
		cw_assert(ch_get_iterate(ch, (void **)&k, (void **)&v));
